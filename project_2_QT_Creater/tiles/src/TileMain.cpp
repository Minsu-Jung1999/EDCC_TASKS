...
void createRadioButtons(GWindow*, GRadioButton**, GRadioButton**, GRadioButton**, 
                        GRadioButton**, GRadioButton**);
GLabel* createLabels(GWindow* window);
void processMouseEvent(GEvent mouseEvent, TileManager* manager, GLabel* glMousePos, 
                        GRadioButton* grbRaise, GRadioButton* grbLower, GRadioButton* grbRemove,
                        GRadioButton* grbRemoveAll, GRadioButton* grbHighlight, GWindow* window);
...
int main() {
    ...
    createRadioButtons(window, &grbRaise, &grbLower, &grbRemove, &grbRemoveAll, &grbHighlight);
    ...
    window->setMouseListener([manager, glMousePos, grbRaise, grbLower, grbRemove, 
                            grbRemoveAll, grbHighlight, window](GEvent e) { 
        processMouseEvent(e, manager, glMousePos, grbRaise, grbLower, grbRemove, grbRemoveAll, 
                            grbHighlight, window);
    });
...
// Takes a pointer to a GUI window and a pointer to a tile manager. Creates TILE_COUNT number
// of tiles of random sizes, positions and colors and adds them to the manager at the bottom
// of the z-ordering. Then, draws  all tiles on the GUI window.
void addInitialTiles(GWindow* window, TileManager* manager) {
...
// Creates and returns a Tile with a random size between the min and max specified and at a 
// random position entirely within the window. It also gives each Tile a random color. 
Tile createTile() {
...
// Initializes three buttons and adds hotkey combinations to select them
 void createButtons(GWindow* window, TileManager* manager) {
    ...
    gbAddFront->setClickListener([manager, window]() {
       Tile t = createTile();
       manager->addTop(t);
       manager->drawAll(*window);
    });
    ...
    gbAddBack->setClickListener([manager, window]() {
       Tile t = createTile();
       manager->addBottom(t);
       manager->drawAll(*window);
    });
    ...
    gbClear->setClickListener([manager, window]() {
       window->clear();
       manager->clear();
    });
...
void createSingleRadio(GWindow* window, std::string text, std::string key, 
...
void createRadioButtons(GWindow* window, GRadioButton** grbRaise, GRadioButton** grbLower, 
                        GRadioButton** grbRemove, GRadioButton** grbRemoveAll, 
                        GRadioButton** grbHighlight) {
...
// This function runs when any mouse event occurs. It gets passed an event parameter
// from which we can access the x, y position of the event. Prints the current x, y
// location of the mouse to the label on the gui. Then calls the appropriate student 
